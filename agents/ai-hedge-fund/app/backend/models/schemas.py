...
def _default_model_name() -> str:
    return os.getenv("AIF_DEFAULT_MODEL", "grok-4-0709")


def _default_model_provider() -> ModelProvider:
    provider = os.getenv("AIF_DEFAULT_PROVIDER", "xAI").strip().lower()
    for p in ModelProvider:
        if p.value.lower() == provider or p.name.lower() == provider:
            return p
...
class BaseHedgeFundRequest(BaseModel):
    ...
    model_name: Optional[str] = Field(default_factory=_default_model_name)
    model_provider: Optional[ModelProvider] = Field(default_factory=_default_model_provider)
...
class DexterChatRequest(BaseModel):
...
class DexterChatResponse(BaseModel):
    response: str
