...
def create_graph(graph_nodes: list, graph_edges: list) -> StateGraph:
    ...
    # Add devil's advocate nodes (one per portfolio manager group, matching suffix pattern)
    devils_advocate_nodes = {}  # Map portfolio manager ID to devil's advocate ID
    for portfolio_manager_id in portfolio_manager_nodes:
        suffix = portfolio_manager_id.split('_')[-1]
        devils_advocate_id = f"devils_advocate_agent_{suffix}"
        devils_advocate_nodes[portfolio_manager_id] = devils_advocate_id
        devils_advocate_function = create_agent_function(devils_advocate_agent, devils_advocate_id)
        graph.add_node(devils_advocate_id, devils_advocate_function)
    ...
    # Connect analysts → devil's advocate (instead of directly to risk manager)
    for analyst_id, portfolio_manager_id in direct_to_portfolio_managers.items():
        devils_advocate_id = devils_advocate_nodes[portfolio_manager_id]
        graph.add_edge(analyst_id, devils_advocate_id)

    # Connect devil's advocate → risk manager → portfolio manager
    for portfolio_manager_id in portfolio_manager_nodes:
        devils_advocate_id = devils_advocate_nodes[portfolio_manager_id]
        risk_manager_id = risk_manager_nodes[portfolio_manager_id]
        graph.add_edge(devils_advocate_id, risk_manager_id)
