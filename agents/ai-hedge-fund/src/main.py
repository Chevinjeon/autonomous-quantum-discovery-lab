...
def create_workflow(selected_analysts=None):
    ...
    # Always add devil's advocate, risk, and portfolio management
    workflow.add_node("devils_advocate_agent", devils_advocate_agent)
    workflow.add_node("risk_management_agent", risk_management_agent)
    workflow.add_node("portfolio_manager", portfolio_management_agent)

    # Connect: analysts → devil's advocate → risk management → portfolio manager
    for analyst_key in selected_analysts:
        node_name = analyst_nodes[analyst_key][0]
        workflow.add_edge(node_name, "devils_advocate_agent")
